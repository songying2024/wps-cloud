...
export function apply(ctx, config) {
  ...
  async function ensureAuthorized() {
    ...
    try {
      // 探活：最小请求验证会话有效性
      await cli.run('drive', 'list-my-files', { page_size: 1 }, { timeout: 15000 })
    } catch (error) {
    ...
    async execute(args) {
      ...
      try {
        ...
        if (!ok) {
          return {
            nextStep: '自动登录未完成',
            detail: '抓取到的会话无法访问云文档下载，请重试登录。也可改用手动回传 sid 方式：在浏览器登录 https://365.kdocs.cn 后，把 wps_sid 通过本工具的 sid 参数回传。',
          }
        }
        auth.save({ sid, savedAt: Date.now(), user: nickname || undefined })
        const { driveId, userName } = await finalizeAuthorization()
