...
class WpsSidAuth {
  ...
  /** 用户是否显式退出过（退出后不再自动从密钥链恢复，直到重新登录）。 */
  loggedOut() {
    return Boolean(this.state.loggedOut)
  }

  markLoggedOut() {
    this.save({ loggedOut: true })
  }

  clearLoggedOut() {
    if (this.state.loggedOut) this.save({ loggedOut: undefined })
  }
...
class KdocsCli {
  ...
  candidatePaths() {
    ...
    // 灵犀桌面自带的 kdocs-cli（GUI 启动器环境下无 KDOCS_CLI_PATH 变量时的关键兑底）
  ...
  run(service, action, params = {}, options = {}) {
    ...
    return new Promise((resolvePromise, rejectPromise) => {
      ...
      child.on('close', (code) => {
        ...
        try {
          const parsed = this.parseOutput(stdout, label(service, action))
          rateLimitState.lastHitAt = 0  // 调用成功说明配额已恢复
          resolvePromise(parsed)
        } catch (error) {
          if (isRateLimitError(error)) rateLimitState.lastHitAt = Date.now()
          rejectPromise(error)
        }
      })
    })
...
async function captureSidViaBrowser(browserPath, config) {
  ...
  try {
    ...
    // 抓到 wps_sid 即返回，不再用 verifySid 验证卡死：
    // 浏览器登录的 365.kdocs.cn 会话 sid 与 kdocs-cli 引擎密钥链 sid 常非同会话（下载 403），
    // 在 captureSidViaBrowser 内验证会必然失败并空等至超时。
    // sid 的有效性由调用方处理：wps_authorize 二次 verifySid + 密钥链兜底，
    // login 路由 / maybeOpenLogin 统一走 persistLoginSid（密钥链 sid 优先）。
...
/** 判断错误是否属于「限流/配额用尽」（非登录问题，不应误报未登录）。 */
function isRateLimitError(error) {
  const text = String(error && error.message ? error.message : error)
  return /调用次数已达上限|次数上限|429|rate.?limit|quota|将于.*恢复/i.test(text)
}

/** 最近一次引擎限流命中时间（0 = 无）；status 路由据此透出 rateLimited，零引擎调用。 */
const rateLimitState = { lastHitAt: 0 }
...
export function apply(ctx, config) {
  ...
  function maybeOpenLogin(force) {
    ...
    captureSidViaBrowser(browser, config)
      .then(({ sid, nickname }) => {
        // 优先密钥链同会话 sid，浏览器 sid 仅是身份凭证
        persistLoginSid(sid, nickname)
      })
      .catch(() => {
        persistLoginSid('')
      })
  ...
  /** 登录成功后持久化会话：优先采用密钥链里与引擎同会话的 sid（V02ShV），浏览器抓取的 sid 只是身份凭证。 */
  function persistLoginSid(browserSid, nickname) {
    const kcSid = readKeychainSid()
    const finalSid = kcSid || browserSid
    if (finalSid) {
      auth.save({ sid: finalSid, savedAt: Date.now(), user: nickname || undefined })
    }
    return finalSid
  }

  async function tryRestoreFromKeychain() {
    // 用户显式退出过（wps_logout）后不再自动恢复，需重新授权登录
    if (auth.loggedOut()) return false
    const kcSid = readKeychainSid()
    if (!kcSid) return false
    // 密钥链里的 wps_sid 是 kdocs-cli 引擎自己保存、与引擎同会话的凭证，直接采信即可。
    // 不做真实下载验证：verifySid 会消耗每日配额，且配额耗尽(429)会被误判为“sid 无效”，
    // 导致退出登录后无法从密钥链恢复、被误报为“未登录”。
    if (kcSid !== auth.sid()) {
  ...
  async function ensureAuthorized() {
    ...
    // 有 sid 时不再额外探活：直接放行，由具体接口的引擎调用结果判定。
    // 原因：探活本身消耗 kdocs-cli 每日配额，且限流(429)会被误判为未登录，
    // 触发弹登录窗→重新登录→再探活的死循环，浪费配额并让用户以为登录坏了。
    // 认证失效由具体工具调用捕获 isAuthError 时再处理。
    return
  }
      ...
      if (auth.hasSid()) {
        try {
          ...
          if (isRateLimitError(error)) {
            // 配额耗尽不等于未登录：本地 sid 仍在，登录态正常
            authorized = true
            loginHint = `\n今日接口调用配额已用完（${error.message}）。登录状态本身正常，次日自动恢复，无需重新登录。`
  ...
  register({
    ...
    async execute(args) {
      ...
      try {
        ...
        // 二次确认浏览器抓取的 sid 是否与引擎同会话；无效则改从密钥链恢复同会话 sid
  ...
  register({
    ...
    async execute() {
      auth.clear()
      auth.markLoggedOut()
      return { ok: true, detail: '已退出登录：本地会话已清除，自动恢复已关闭。需重新登录（wps_authorize）才能继续使用云文档。' }
    },
  })
    ...
    ctx.effect(() => {
      ...
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          // status 不做引擎探活（设置页每 2 秒轮询，探活会烧 kdocs-cli 每日配额）。
          // 逻辑：有本地 sid 即视为已登录；若密钥链 sid 不同则静默替换（同会话下载签名匹配）。
          // 真实可用性由实际操作（列文件等）体现，其错误消息已能准确传达认证/限流状态。
          if (auth.hasSid()) {
            try {
              const kcSid = readKeychainSid()
              if (kcSid && kcSid !== auth.sid()) {
          ...
          status.rateLimited = rateLimitState.lastHitAt > 0
            && (Date.now() - rateLimitState.lastHitAt) < 24 * 3600 * 1000
      ...
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          captureSidViaBrowser(browser, config)
            .then(({ sid, nickname }) => {
              // 优先密钥链同会话 sid（浏览器抓取的 sid 常与引擎非同会话，下载 403）
              persistLoginSid(sid, nickname)
            })
            .catch(() => {
              // 浏览器抓取失败/超时：密钥链里已有同会话 sid 时照样完成登录
              persistLoginSid('')
            })
