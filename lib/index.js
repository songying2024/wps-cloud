...
export function apply(ctx, config) {
  ...
  async function ensureAuthorized() {
    ...
    try {
      // 探活：用真实下载验证 sid 是否与引擎同一会话
      const ok = await verifySid(auth.sid())
      if (!ok) {
        // 当前 sid 无效（可能是插件抓取的非同会话 sid）：尝试从密钥链恢复
        if (await tryRestoreFromKeychain()) return
        auth.clear()
        const hint = unauthorizedHint()
        throw new Error(`WPS: 登录已失效。${hint}`)
      }
    } catch (error) {
  ...
  register({
    ...
    async execute(args) {
      ...
      const size = await downloadToFile(url, finalPath, '文件', auth.sid())
    ...
    ctx.effect(() => {
      ...
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (auth.hasSid()) {
            try {
              // 真正验证 sid 本身是否可用（用引擎拿真实文件下载探活），避免引擎 token 有效而 sid 失效的误判
              let ok = await verifySid(auth.sid())
              if (!ok) {
                // 当前 sid 无效：尝试从密钥链恢复与引擎同一会话的有效 sid
                ok = await tryRestoreFromKeychain()
              }
              if (ok) {
                await finalizeAuthorization()
      ...
      disposers.push(webServer.register({
        ...
        handler: async (req, res) => {
          ...
          try {
            ...
            mkdirSync(uploadsDir, { recursive: true })
            const dest = join(uploadsDir, `${Date.now()}-${safeName}`)
            const size = await downloadToFile(dlUrl, dest, '文件', auth.sid())
