...
`;
    ...
    function WpsSettingsPage() {
      ...
      const statusText = state.loading
        ? "正在检查…"
        : state.authorized
          ? (state.rateLimited ? "已登录（今日接口配额已用完，明日自动恢复）" : "已登录")
          : "未登录";
      ...
      if (state.authorized) {
        ...
        if (state.rateLimited) {
          rows.push(h("div", { className: "wpsc-row", key: "quota" }, "今日金山接口调用配额已用完（多次登录验证/浏览会消耗配额），将于明早自动恢复。登录状态本身正常，无需重新登录。"));
        }
    ...
    function WpsCloudPickerModal({ onClose, onPick }) {
      ...
      const loadItems = react.useCallback(async (parentId) => {
        ...
        try {
          ...
          const isQuota = /调用次数已达上限|次数上限|429/.test(msg);
          setError(isQuota ? msg + "（登录状态本身正常，明早自动恢复，无需重新登录）" : msg);
          setItems([]);
        } finally {
          setLoading(false);
        }
